...
def problem_five(request):
    Student.objects.create(id = 38, first_name = 'Kristoff', last_name = 'Dixon', year = 12, gpa = 4.33)
    student = Student.objects.get(id = 38)
...
def problem_six(request):
    Student.objects.filter(id = 38).update(gpa = 4.00)
    student = Student.objects.get(id = 38)
    print(f'Id: {student.id}')
    print(f'Full Name: {student.first_name} {student.last_name}')
    print(f'GPA: {student.gpa}')
...
def problem_seven(request):
    Student.objects.filter(id = 38).delete()
    # Make sure to set this equal to the primary key of the row you just created!
    student_id = 38
